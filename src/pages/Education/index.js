...
const Education = (p) => {
    ...
    //         loader(true)
    //         ApiClient.delete('delete?model=employment&id=${id}').then(res => {
    //             if (res.success) {
    ...
    const deleteItem = (id) => {
        if (window.confirm("Do you want to delete this")) {
            loader(true)
            ApiClient.delete(`delete?model=education&id=${id}`).then(res => {
                if (res.success) {
    ...
    const view=(id)=>{
        history.push("/educationDetail/"+id)
    }

    const edit=(id)=>{
        let url=`/education/edit/${id}`
        history.push(url)
    }
