...
const Roles = (p) => {
    ...
    const getData = (p = {}) => {
        ...
        ApiClient.get('api/user/roles/listing', filter).then(res => {
            if (res.success) {
    ...
    const deleteItem = (id) => {
        if (window.confirm("Do you want to delete this")) {
            loader(true)
            ApiClient.delete('api/user/roles/delete', {id: id }).then(res => {
                if (res.success) {
    ...
    const statusChange=(itm)=>{
        ...
        if(window.confirm(`Do you want to ${status=='active'?'Activate':'Deactivate'} this`)){
            loader(true)
            ApiClient.put(`api/user/roles/status/change`,{id:itm.id,status}).then(res=>{
                if(res.success){
                    getData()
                }
                loader(false)
